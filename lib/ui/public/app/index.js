  ...
  angular.module('app', ['Pagination'])
    ...
    .controller('Page1Ctrl', ['$rootScope', '$scope', '$http', '$timeout', function ($rootScope, $scope, $http, $timeout) {
      ...
      if (state.action === 'is_unique') {
        ...
        $http.get(`lists?apiKey=${config.credential.token}`)
    ...
    .controller('Page4Ctrl', ['$scope', '$rootScope', '$http', function ($scope, $rootScope, $http) {
      ...
      var buildRhv = function (unit, increment) {
        return '{{format submission.timestamp operator="subtract" ' + unit + '="' + increment + '"}}-{{submission.timestamp}}';
      };

      // Finalization and communicating to the user what's next
      $scope.finish = function () {
        $scope.finishing = true;
        $http.post(`lists/ensure?apiKey=${config.credential.token}`, {
          ...
          if (state.ttl) {
            ...
            if (state.ttl === 'custom') {
              ...
              rhv = buildRhv(amount.unit, amount.increment);
            }
